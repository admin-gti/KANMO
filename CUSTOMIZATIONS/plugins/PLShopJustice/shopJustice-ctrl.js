var shopJustice = ["$scope", "$http", "ModelService", "$stateParams", "prismSessionInfo", "$location", "$modalInstance", "$modal", "XZOutReporter", "$state", "base64", "$q", "PrintersService", "PrismUtilities", "$filter", "$window", "ResourceNotificationService", "LoadingScreen", "NotificationService", "sharedProp",
    function($scope, $http, ModelService, $stateParams, prismSessionInfo, $location, $modalInstance, $modal, XZOutReporter, $state, base64, $q, PrintersService, PrismUtilities, $filter, $window, rNS, LoadingScreen, NS, sharedProp) {
        'use strict';
        ...
        var magentoAPI = function() {
                ...
                $http.get(servername + root_url + "getAccessToken", { headers: { "Auth-Session": sess } })
                    .success(function(response) {
                        ...
                        if (obj.status_code == "200") {
                            ...
                            $scope.getCustomerEmail();
                        } else {
                            LoadingScreen.Enable = !1;
                            if (obj.status_code == "0") {
                                NS.addAlert('Network connection error. Possible cause no internet connection or slow internet speed.', 'Network Error', 'static', false).then(function() {
                                    $modalInstance.dismiss();
                                    $state.go($state.current, {}, { reload: true });
                                });
                            } else {
                                NS.addAlert('Unable to retrieve external resources!', 'External Error', 'static', false).then(function() {
                                    $modalInstance.dismiss();
                                    $state.go($state.current, {}, { reload: true });
                                });
                            }
            ...
            $scope.getCustomerEmail = function() {
                $http.get(servername + root_url + "getCustomerEmail&email=" + activeCustomer.email_address, { headers: { "Auth-Session": sess } })
                    .success(function(response) {
                        var obj = JSON.parse(response);
                        if (obj.status_code == "200" && obj.length > 0) {
                            $scope.getCartItems(obj.result.items[0].id);
                            sharedProp.setMainCustomerId(obj.result.items[0].id);
                        } else {
                            LoadingScreen.Enable = !1;
                            if (obj.status_code == 0) {
                                NS.addAlert('Network connection error. Possible cause no internet connection or slow internet speed.', 'Network Error', 'static', false).then(function() {
                                ...
                                NS.addAlert('Unable to retrieve customer details!', 'External Error', 'static', false).then(function() {
                                    $modalInstance.dismiss();
                                    $state.go($state.current, {}, { reload: true });
                                });
                            }
                                        ...
                                        return ModelService.get('Inventory', { filter: filter })
                                            .then(function(res) {
                                                if (res.length > 0) {
                                                    ...
                                                    obj.result.items[key].prism_item.item_id = value.item_id;
        ...
        $scope.loadCustomer = function() {
            var modalOptions = {
                ...
                // windowClass: 'sm',
                size: 'lg', // sm, md, lg
                templateUrl: '/plugins/PLShopJustice/customerList.htm',
        ...
        $scope.$watch(function () { return sharedProp.getGRItems();}, function (value) {
              $scope.giftRegistryItems = value;
          });
...
window.angular.module('prismPluginsSample.controller.shopJusticeCtrl', [])
    ...
    .service('sharedProp', function() {
        var mainCustomerId, selectedCustomerId, customerName, giftRegistryItems;
        return {
            getSelectedCustomerId: function() {
                return selectedCustomerId;
            },
            setSelectedCustomerId: function(value) {
                selectedCustomerId = value;
            },
            getMainCustomerId: function() {
                return mainCustomerId;
            },
            setMainCustomerId: function(value) {
                mainCustomerId = value;
            },
            getSelectedCustomerName: function() {
                return customerName;
            },
            setSelectedCustomerName: function(value) {
                customerName = value;
            },
            setGRItems: function(value){
            	giftRegistryItems = value;
            },
            getGRItems: function(){
...
// Customer List
var customerList = ["$scope", "$http", "ModelService", "$stateParams", "prismSessionInfo", "$location", "$modalInstance", "$modal", "XZOutReporter", "$state", "base64", "$q", "PrintersService", "PrismUtilities", "$filter", "$window", "ResourceNotificationService", "LoadingScreen", "NotificationService", "sharedProp",
    function($scope, $http, ModelService, $stateParams, prismSessionInfo, $location, $modalInstance, $modal, XZOutReporter, $state, base64, $q, PrintersService, PrismUtilities, $filter, $window, rNS, LoadingScreen, NS, sharedProp) {
        'use strict';
        var sess = $http.defaults.headers.common['Auth-Session'];
        var servername = $window.location.origin;
        ...
        $scope.searchCustomer = function() {
            if ($scope.searchType == '' || $scope.keyword == '') {
                rNS.showError('Error', 'Please select search type and enter keyword!');
            } else {
                LoadingScreen.Enable = 1, $http.get(servername + '/plugins/PLShopJustice/transactions_request.php?action=getCustomerDetails&searchType=' + $scope.searchType + '&keyword=' + $scope.keyword).then(function(result) {
                    LoadingScreen.Enable = 0;
                    var jsonObj = JSON.parse(result.data);
                    if (jsonObj.status_code == 200) {
                        $scope.customers = jsonObj.result.items;
                        $scope.totalCount = jsonObj.result.total_count;
                    } else {
        ...
        $scope.getGiftRegistryItems = function(customer_id, customerName) {
            sharedProp.setSelectedCustomerId(customer_id);
            sharedProp.setSelectedCustomerName(customerName);
            var modalOptions = {
                backdrop: 'static',
                // windowClass: 'sm',
                size: 'lg', // sm, md, lg
                templateUrl: '/plugins/PLShopJustice/giftRegistryItems.htm',
                controller: 'giftRegistryCtrl',
                keyboard: false
            };
...
window.angular.module('prismPluginsSample.controller.customerListCtrl', [])
    .controller('customerListCtrl', customerList);

// Gift Registry Items list
var giftRegistryList = ["$scope", "$http", "ModelService", "$stateParams", "prismSessionInfo", "$location", "$modalInstance", "$modal", "XZOutReporter", "$state", "base64", "$q", "PrintersService", "PrismUtilities", "$filter", "$window", "ResourceNotificationService", "LoadingScreen", "NotificationService", "sharedProp", "$modalStack",
    function($scope, $http, ModelService, $stateParams, prismSessionInfo, $location, $modalInstance, $modal, XZOutReporter, $state, base64, $q, PrintersService, PrismUtilities, $filter, $window, rNS, LoadingScreen, NS, sharedProp, $modalStack) {
        'use strict';
        var sess = $http.defaults.headers.common['Auth-Session'];
        var servername = $window.location.origin;
        var activeCustomer = ModelService.fromCache('Customer')[0];
        ...
        $scope.customerName = sharedProp.getSelectedCustomerName();
            ...
            $scope.getAccessToken = function() {
                $http.get(servername + root_url + "getAccessToken", { headers: { "Auth-Session": sess } })
                    .success(function(response) {
                        ...
                        if (obj.status_code == "200") {
                            sessionStorage.setItem("items_access_token", true);
                            $scope.getGiftRegistryItems(sharedProp.getSelectedCustomerId());
                        } else {
                            LoadingScreen.Enable = !1;
                            if (obj.status_code == "0") {
                                NS.addAlert('Network connection error. Possible cause no internet connection or slow internet speed.', 'Network Error', 'static', false).then(function() {
        ...
        $scope.saveSelectedItems = function() {
            ...
            if (typeof $scope.giftRegistryItems !== 'undefined') {
                ...
                if(sharedProp.getMainCustomerId() == sharedProp.getSelectedCustomerId())
                {
                	$q.all(grItems).then(function(){
                		$scope.giftRegistryItems.result.items = grItems;
                		$scope.giftRegistryItems.length = grItems.length;
                		sharedProp.setGRItems($scope.giftRegistryItems);
                	});
                }
...
window.angular.module('prismPluginsSample.controller.giftRegistryCtrl', [])
    .controller('giftRegistryCtrl', giftRegistryList);