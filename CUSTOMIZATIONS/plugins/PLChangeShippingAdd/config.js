SideButtonsManager.addButton({
    ...
    handler: ['$q', 'DocumentPersistedData', 'NotificationService', '$modal', 'Templates', 'ModelService', '$rootScope', 'HookEvent', '$stateParams', '$http', '$window', 'LoadingScreen', '$interval', '$state', function($q, DocumentPersistedData, NotificationService, $modal, Templates, ModelService, $rootScope, HookEvent, $stateParams, $http, $window, LoadingScreen, $interval, $state) {
        ...
        ModelService.get('Document', {sid: activeDocumentSid}).then(function(doc){
            ...
            if(doc[0].bt_cuid == '' || doc[0].bt_cuid == null)
            {
                ModelService.get('Item', {document_sid: activeDocumentSid}).then(function(items){
                    ...
                    LoadingScreen.Enable = !0, $http.post(servername + '/plugins/PLCRM/crmFunctions.php', {action: 'retrieveCustomerData'}).then(function(result){
