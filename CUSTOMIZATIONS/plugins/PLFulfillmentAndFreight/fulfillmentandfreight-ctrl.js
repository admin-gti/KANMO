var fulfillmentandFreightCtrl = ["$scope", "$http", "prismSessionInfo", "$location", "$modalInstance", "$modal", "XZOutReporter", "$state", "base64","$q","PrintersService","PrismUtilities","ModelService","$filter","$window", "$stateParams","LoadingScreen","ResourceNotificationService","$modalInstance","$modalStack","NotificationService",
    function(a, b, c, d, e, f, k, m, n, o, ps, pu, ms, $filter,$window, $stateParams, i, NotificationService,$modalInstance,$modalStack,nt) {
...
//                              a.currentStore = 'PIJ';
    ...
    a.update = function(){
        ...
        modalOptions = {
            ...
            windowClass: 'medium',
            size: 'md',
            templateUrl: '/plugins/PLFulfillmentAndFreight/provisionalMsg.htm',
            controller: 'provisionalMsgCtrl'
        };
...
//            i.Enable = !1; 
    ...
    a.SelectAddress = function(ret, index){
        if(ret == 1){
            $("#defaultadd"+index).fadeIn('fast');
            $("#customadd"+index).fadeOut('fast');
        }else if(ret == 2){
            $("#defaultadd"+index).fadeOut('fast');
            $("#customadd"+index).fadeIn('fast');
        }
        $("#other_shipping"+index).fadeOut('fast');
    }
