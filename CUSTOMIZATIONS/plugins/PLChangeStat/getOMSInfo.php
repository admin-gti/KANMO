...
//            $conn->debug = TRUE;
            ...
            foreach($data['Orders'][0]['OrderLineId'] as $key => $value) {
                ...
                if($req['sid'] == 0) {
                    $sql = "SELECT a.sid from document_item a LEFT JOIN document b ON (a.doc_sid = b.sid) where notes_order = '".$req['ref']."'";
                    $rsResult1 = $conn->Execute($sql);
                } else {
                    $sql = "SELECT sid from document_item where doc_sid = '".$req['sid']."'";
                    $rsResult1 = $conn->Execute($sql);
                }
                ...
                while(!$rsResult1->EOF){
                    ...
                    $sql = "UPDATE "
                            ...
                            . "WHERE alu = '".$value['SKU']."' "
                            . "and doc_sid = '".$rsResult1->fields['sid']."'";
//                    $conn->debug = TRUE;
                    $conn->Execute($sql);
