ButtonHooksManager.addHandler(['after_posLoyalty'],
    function($q, DocumentPersistedData, NotificationService, $modal, Templates, ModelService,$rootScope,HookEvent, $stateParams, $http, $window, LoadingScreen, $interval, $state) {
        ...
        var modalOptions = {
                ...
                // windowClass: 'sm',
                size: 'md', // sm, md, lg
                templateUrl: '/plugins/PLLoyalty/index.htm',
        ...
        var checkNetworkStatus = function()
        {
            ...
            $http.get(servername + '/plugins/PLLoyalty/loyalty.php?action=checkNetworkStatus').then(function(result){
        ...
        LoadingScreen.Enable = !0,checkNetworkStatus().then(function(){
        	if(is_online)
        	{
        		ModelService.get('Document', {sid: activeDocumentSid}).then(function(doc){
        			if(doc[0].transaction_total_amt > 0)
        			{
        			...
        			{
        				LoadingScreen.Enable = !1;
        				NotificationService.addAlert('This transaction has already 100% discounted!', 'Points Validation', 'static', false).then(function(){
        				    $state.go($state.current, {}, {reload: true});
        				    deferred.reject();
        				});
        			}
...
var loyaltyCM = ['ModelEvent', '$stateParams', 'ModelService', '$http', '$window', '$state', function(ModelEvent, $stateParams, ModelService, $http, $window, $state){
    var servername        = $window.location.origin;
    ...
    var handlerAfterItemInsertRemove = function($q, item){
        ...
        {
            var loyaltyItems = JSON.parse(sessionStorage.getItem('loyaltyItems'));
            var params = [{"Params":{"spreadto":loyaltyItems.transaction_type,"discountreason":"LOYALTY POINTS","sid": activeDocumentSid},"MethodName":"SpreadDocumentDiscount"}];
            $http.post(servername + '/v1/rpc', params).then(function(){
                //resolve the deferred opteration
                deferred.resolve();
            });
...
ConfigurationManager.addHandler(loyaltyCM);