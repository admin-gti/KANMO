  	...
  	function($scope, $http, ModelService, $stateParams, prismSessionInfo, $location, $modalInstance, $modal, XZOutReporter, $state, base64, $q, PrintersService, PrismUtilities, $filter, $window, RN, LoadingScreen, DocumentPersistedData, NotificationService) {
  		...
  		$scope.applyCard = function(){
  			if($scope.gcNumber != '' && $scope.gcNumber != null && typeof $scope.gcNumber != 'undefined')
  			{
  				...
  				{
    				LoadingScreen.Enable = 1, $http.get(servername + '/plugins/PLGiftCard/giftCard.php?action=checkNetworkStatus').then(function(result){
    				    if(result != '' && result != 'false')
    				    {
				    	    ...
				    	    {
				    	    	$http.get(servername + '/plugins/PLGiftCard/giftCard.php?action=checkGCStatus&cardNumber='+ $scope.gcNumber).then(function(result){
				    	    		...
				    	    		{
				    	    			NotificationService.addAlert('Your network connection is too slow!', 'Request Timeout', 'static', false);
    				    ...
    				    {
    				    	NotificationService.addAlert('Network problem occured. Please check your network connection!', 'Network Error', 'static', false).then(function(){
