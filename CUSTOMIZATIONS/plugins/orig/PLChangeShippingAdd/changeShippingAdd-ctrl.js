var changeShippingAdd = ["$scope", "$http", "ModelService", "$stateParams", "prismSessionInfo", "$location", "$modalInstance", "$modal", "XZOutReporter", "$state", "base64","$q","PrintersService","PrismUtilities","$filter","$window", "ResourceNotificationService", "LoadingScreen",
 			...
 			if(validate.indexOf('0') === -1)
 			{
	 			angular.forEach(itemToUpdate, function(item){
	 				chainRequest = chainRequest.then(function(){
						...
						var selected_add_sid = item.note1;
	 			...
	 			NotificationService.showSuccessfulMessage('Updated', 'Shipping address has been updated');
	 			$modalInstance.dismiss();
	 			$state.go($state.current, {}, {reload: true});
 			}
 			else
 			{
 				NotificationService.showError('Error', 'Please fill all item(s) addresses!');
 			}
